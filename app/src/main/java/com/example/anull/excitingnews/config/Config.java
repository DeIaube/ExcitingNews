package com.example.anull.excitingnews.config;

/**
 * Created by null on 2016/7/18.
 */
public class Config {
    public static final boolean DEBUG = true;
    public static final String BASE_URL = "http://news-at.zhihu.com/";
    public static final String LATEST_URL = "api/4/news/latest";
    public static final String DETAIL_URL = "api/4/news/{id}";
    public static final String BEFORE_URL = "api/4/news/before/{data}";

    public static final String CLOUD_ID = "ooU0OqhYdkP0tPaIYW3Jb0TD-gzGzoHsz";
    public static final String CLOUD_KEY = "NkugqhYY1c2hlUp3uIf6IPV0";

    public static String ID = "";

}
